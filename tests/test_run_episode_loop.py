...
from tools import run_episode_loop as rel
...
def test_detects_episode_when_home_and_label_present(monkeypatch):
    ...
    assert rel._detect_current_episode(device=None, store=None) == 4


def test_returns_none_when_home_not_clean(monkeypatch):
    _patch(monkeypatch, home_found=False, label_found=True, ocr_value=4)
    assert rel._detect_current_episode(device=None, store=None) is None
...
def test_order_is_optional_now():
    ...
    ap.add_argument("--order", type=rel._episode_list, default=None)
