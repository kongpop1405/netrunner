...
## ผลวัดจาก device ที่ bot กำลังรัน = โมฆะ ห้ามเอาไปสรุปสาเหตุ

Section ด้านบนกับ "Kill bot ได้เลย" พูดถึง **crash** และ **จอสู้กัน** แต่ยังมีโหมดพังที่เงียบกว่านั้น: **การทดลองที่รันสำเร็จทุกขั้น ไม่ error อะไรเลย แต่ตัวเลขที่ได้เป็นของสถานการณ์ที่ bot สร้าง ไม่ใช่ของสิ่งที่กำลังทดสอบ** — แล้วมันอ่านเหมือนหลักฐานชั้นดี เพราะ "ทำซ้ำได้"

เจอจริง 2026-08-18: ไล่หาว่าทำไม `switch_episode` กดปุ่ม Episode ไม่ติด → เขียน repro ยิง swipe แล้ว tap ที่ delay 0s/1s/2.5s → **"swallowed" ทั้ง 3 ครั้ง** สรุปทันทีว่า scroll momentum กินทัพ. ของจริงคือ **bot กำลังรันแย่งกดจออยู่** — พอ kill bot แล้วยิงชุดเดิม เปิด map ได้ `1.000` ทั้ง control และ after-swipe ⇒ สมมติฐานทั้งอันเป็นขยะ และเสียเวลาไล่ผิดทาง 2 รอบ (โทษ momentum → โทษ marker stale) กว่าจะเจอสาเหตุจริง (state leak — จอไม่ใช่ home)

- **ก่อนอ้างผลวัดจาก device ต้อง kill bot ก่อนเสมอ** ไม่ใช่แค่ตอนจะแตะจอ — รวมถึงตอน "แค่อ่านค่า/แค่ยิง repro" ด้วย
- **สัญญาณว่าผลอาจปนเปื้อน**: repro ให้ผลเหมือนกันทุก delay (ตัวแปรที่ตั้งใจเปลี่ยนไม่มีผลเลย) · จอเปลี่ยนเองระหว่างวัด · เจอ `home_play_marker` หายไปทั้งที่ไม่ได้กดอะไร
- **ต้องมี control ที่รู้คำตอบล่วงหน้า** — ยิงเคสที่ "ต้องผ่าน" คู่กับเคสที่สงสัยทุกครั้ง. ถ้า control ก็พังด้วย = harness ผิด ไม่ใช่ของที่เทสผิด
- **เขียนกำกับในรายงานว่าวัดตอน bot หยุดหรือรัน** — ผลที่ไม่ระบุสภาพแวดล้อม เอากลับมาใช้ยืนยันอะไรทีหลังไม่ได้

## Marker ที่ crop จากเฟรมมี overlay คลุม = under-score เงียบทุกเฟรม

Template ที่ตัดมาจากเฟรมซึ่งมี **scrim/dim ของ dialog คลุมอยู่** จะ match ของจริง (ที่ไม่มี scrim) ได้คะแนนต่ำค้างตลอด — ไม่ error, ไม่ 0, แค่ต่ำกว่า threshold นิดเดียวทุกครั้ง แล้ว detector คืน `None` ทุกเฟรม

เจอจริง 2026-08-18: `home/episode_label_marker.png` ถูก crop จากเฟรมที่ "Connection lost!" คลุมอยู่ (แถบป้ายมืด) + ตำแหน่งเยื้องขวาจนตัดตัวอักษร → ได้ **0.690 ทุกเฟรมจริง** ต่ำกว่า threshold 0.82 ⇒ `detect_current_episode()` คืน `None` เสมอ ⇒ guard ที่เขียนถูกต้อง (ปฏิเสธสลับ episode ถ้าอ่านค่าเดิมไม่ได้) **ทำให้ฟีเจอร์ทั้งอันตายเงียบ: ยิง 5 ครั้ง skip 5 ครั้ง** ไม่มี error ใน log เลย

- **วัดความสว่างก่อน crop** — เฟรมสะอาดกับเฟรมถูกคลุมต่างกันชัด (วัดจริง: strip mean **66** vs **25**) เก็บเฟรมหลายอันแล้วเอาอันสว่างสุด
- **สกัดขอบข้อความด้วย HSV mask ไม่ใช่กะด้วยตา** — `cv2.inRange` จับสีตัวอักษร → `nonzero()` หา bbox → ดู column profile แยก "คำ" กับ "ตัวเลข" (เจอจริง: 7 run ของคำ "Episode" จบที่ x 485 แล้วเลขอยู่ 492-508 → ตั้ง digit ROI จากช่องว่างนั้น)
- **คะแนนที่ค้างเท่ากันเป๊ะทุกเฟรม (เช่น 0.690 ซ้ำ 60 ครั้ง) = template ผิด ไม่ใช่จอสั่น** — ของที่สั่นจริงคะแนนจะแกว่ง
- **`None` จาก detector ที่มี guard ครอบ = ฟีเจอร์ตายแบบไม่มีเสียง** — ต้องนับด้วยว่า guard ยิงกี่ครั้ง (`grep -c 'cannot read'`) ไม่ใช่ดูแค่ว่าไม่มี error
