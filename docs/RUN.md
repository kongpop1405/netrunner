...
**At 0 hearts (added 2026-08-08, all `boxrun_*`/`coinrun`/`xpstat` configs)**: instead of idling in a 30s self-loop, `check_heart` runs the mailbox errand and comes straight back — 0 hearts means 25+ min of dead air before regen, so the wait is put to use on friend-list upkeep:
...
2. ~~`run_config config/cookierun/sendlife.json`~~ — **removed 2026-08-16 (user request)**. The home-screen Friends-tab sweep walks a scrollable list one friend at a time; on a long list it took 26 minutes and tripped `check_heart`'s own `match_timeout` (1,554,406ms >= 1,500,000ms), because `run_config` blocks inside the calling state. `sendlife.json` itself is untouched and still runs standalone via `tools/run_sendlife_loop.py`.
